...
class GalaxeaGearboxAssemblyAgent:
    def __init__(self,
        ...
        # -------------------------------------------------------------------------------------------------------------------------- #
        # [Function] pick_and_twist_insert ----------------------------------------------------------------------------------------- #
        # -------------------------------------------------------------------------------------------------------------------------- #
        self.pick_and_twist_insert_fsm_state = PickAndPlaceState.INITIALIZATION
        self.pick_and_twist_insert_fsm_timer = 0
    ...
    def observe_assembly_state(self):
        ...
        for sun_planetary_gear_idx in range(len(sun_planetary_gear_positions)):
            ...
            horizontal_error = torch.norm(sun_planetary_gear_pos[:, :2] - planetary_carrier_pos[:, :2])
            vertical_error = sun_planetary_gear_pos[:, 2] - planetary_carrier_pos[:, 2]
            orientation_error = torch.acos(torch.clamp((sun_planetary_gear_quat * planetary_carrier_quat).sum(dim=-1), -1.0, 1.0))
    ...
    def pick_and_twist_insert(self,
            object_name: str # sun_gear, ring_gear
        ) -> None:
        # Observe robot state and object state
        self.observe_robot_state()
        self.observe_object_state()
        ...
        print(f"[PICK & PLACE FSM] {self.pick_and_twist_insert_fsm_state}")
        # -------------------------------------------------------------------------------------------------------------------------- #
        # [FSM Start State] INITIALIZATION ----------------------------------------------------------------------------------------- # 
        # -------------------------------------------------------------------------------------------------------------------------- #
        if self.pick_and_twist_insert_fsm_state == PickAndPlaceState.INITIALIZATION:
            # -------------------------------------------------------------------------------------------------------------------------- #
            # Sun gear ----------------------------------------------------------------------------------------------------------------- # 
            # -------------------------------------------------------------------------------------------------------------------------- #
            if object_name == "sun_gear":
            ...
            # [State Transition] INITIALIZATION -> PICK_READY
            self.pick_and_twist_insert_fsm_timer += 1
            if self.pick_and_twist_insert_fsm_timer > TIME_CONSTANT_50:
                self.pick_and_twist_insert_fsm_timer = 0
                self.pick_and_twist_insert_fsm_state = PickAndPlaceState.PICK_READY
        ...
        # [FSM Intermediate State] PICK_READY -------------------------------------------------------------------------------------- #
        # -------------------------------------------------------------------------------------------------------------------------- #
        if self.pick_and_twist_insert_fsm_state == PickAndPlaceState.PICK_READY:
            desired_joint_position, desired_joint_ids = self.solve_inverse_kinematics( 
                arm_name=arm_name,
                target_ee_pos_b=target_pick_ready_pos_b, 
                target_ee_quat_b=target_pick_quat_b
            ...
            self.pick_and_twist_insert_fsm_timer += 1
            if (self.position_reached(ee_pos_w, target_pick_ready_pos_w) and 
                self.pick_and_twist_insert_fsm_timer > TIME_CONSTANT_50 or 
                self.pick_and_twist_insert_fsm_timer > TIME_CONSTANT_150):
                self.pick_and_twist_insert_fsm_timer = 0
                self.pick_and_twist_insert_fsm_state = PickAndPlaceState.PICK_APPROACH

        # -------------------------------------------------------------------------------------------------------------------------- #
        # [FSM Intermediate State] PICK_APPROACH ----------------------------------------------------------------------------------- #
        # -------------------------------------------------------------------------------------------------------------------------- #
        elif self.pick_and_twist_insert_fsm_state == PickAndPlaceState.PICK_APPROACH:
            desired_joint_position, desired_joint_ids = self.solve_inverse_kinematics( 
                arm_name=arm_name,
                target_ee_pos_b=target_pick_pos_b, 
                target_ee_quat_b=target_pick_quat_b
            ...
            self.pick_and_twist_insert_fsm_timer += 1
            if (self.position_reached(ee_pos_w, target_pick_pos_w) and 
                self.pick_and_twist_insert_fsm_timer > TIME_CONSTANT_100 or 
                self.pick_and_twist_insert_fsm_timer > TIME_CONSTANT_150):
                self.pick_and_twist_insert_fsm_timer = 0
                self.pick_and_twist_insert_fsm_state = PickAndPlaceState.PICK_EXECUTION

        # -------------------------------------------------------------------------------------------------------------------------- #
        # [FSM Intermediate State] PICK_EXECUTION ---------------------------------------------------------------------------------- #
        # -------------------------------------------------------------------------------------------------------------------------- #
        elif self.pick_and_twist_insert_fsm_state == PickAndPlaceState.PICK_EXECUTION:
            ...
            # [State Transition] PICK_EXECUTION -> PICK_COMPLETE
            self.pick_and_twist_insert_fsm_timer += 1
            if self.pick_and_twist_insert_fsm_timer > TIME_CONSTANT_100:
                self.pick_and_twist_insert_fsm_timer = 0
                self.pick_and_twist_insert_fsm_state = PickAndPlaceState.PICK_COMPLETE

        # -------------------------------------------------------------------------------------------------------------------------- #
        # [FSM Intermediate State] PICK_COMPLETE ----------------------------------------------------------------------------------- #
        # -------------------------------------------------------------------------------------------------------------------------- #
        elif self.pick_and_twist_insert_fsm_state == PickAndPlaceState.PICK_COMPLETE:
            desired_joint_position, desired_joint_ids = self.solve_inverse_kinematics(
                arm_name        =arm_name,
                target_ee_pos_b =target_pick_ready_pos_b,
                target_ee_quat_b=target_pick_quat_b
            ...
            if (self.position_reached(ee_pos_w, target_pick_ready_pos_w) and 
                ...
                self.pick_and_twist_insert_fsm_timer > TIME_CONSTANT_150):
                self.pick_and_twist_insert_fsm_timer = 0
                self.pick_and_twist_insert_fsm_state = PickAndPlaceState.PLACE_READY

        # -------------------------------------------------------------------------------------------------------------------------- #
        # [FSM Intermediate State] PLACE_READY ------------------------------------------------------------------------------------- #
        # -------------------------------------------------------------------------------------------------------------------------- #
        elif self.pick_and_twist_insert_fsm_state == PickAndPlaceState.PLACE_READY:
            desired_joint_position, desired_joint_ids = self.solve_inverse_kinematics( 
                arm_name=arm_name,
                target_ee_pos_b=target_place_ready_pos_b, 
                target_ee_quat_b=target_place_quat_b
            ...
            if (self.position_reached(ee_pos_w, target_place_ready_pos_w) and 
                ...
                self.pick_and_twist_insert_fsm_timer > TIME_CONSTANT_150):
                self.pick_and_twist_insert_fsm_timer = 0
                self.pick_and_twist_insert_fsm_state = PickAndPlaceState.PLACE_APPROACH

        # -------------------------------------------------------------------------------------------------------------------------- #
        # [FSM Intermediate State] PLACE_APPROACH ---------------------------------------------------------------------------------- #
        # -------------------------------------------------------------------------------------------------------------------------- #
        elif self.pick_and_twist_insert_fsm_state == PickAndPlaceState.PLACE_APPROACH:
            desired_joint_position, desired_joint_ids = self.solve_inverse_kinematics( 
                arm_name=arm_name,
                target_ee_pos_b=target_place_approach_pos_b, 
                target_ee_quat_b=target_place_quat_b
            ...
            if (self.position_reached(ee_pos_w, target_place_approach_pos_w) and 
                ...
                self.pick_and_twist_insert_fsm_timer > TIME_CONSTANT_150):
                self.pick_and_twist_insert_fsm_timer = 0
                self.pick_and_twist_insert_fsm_state = "TWIST_INSERTION"

        # -------------------------------------------------------------------------------------------------------------------------- #
        # [FSM Intermediate State] TWIST_INSERTION --------------------------------------------------------------------------------- #
        # -------------------------------------------------------------------------------------------------------------------------- #
        elif self.pick_and_twist_insert_fsm_state == "TWIST_INSERTION":
            ...
            if self.pick_and_twist_insert_fsm_timer == 0:
                # 현재 팔의 모든 관절 각도 저장
            ...
            alpha = min(self.pick_and_twist_insert_fsm_timer / TWIST_TIME, 1.0)
            ...
            wiggle_offset = wiggle_amplitude * torch.sin(torch.tensor(self.pick_and_twist_insert_fsm_timer * wiggle_freq))
            ...
            self.pick_and_twist_insert_fsm_timer += 1
            if self.pick_and_twist_insert_fsm_timer >= TWIST_TIME:
                self.pick_and_twist_insert_fsm_timer = 0
                self.pick_and_twist_insert_fsm_state = PickAndPlaceState.PLACE_EXECUTION

        # -------------------------------------------------------------------------------------------------------------------------- #
        # [FSM Intermediate State] PLACE_EXECUTION --------------------------------------------------------------------------------- #
        # -------------------------------------------------------------------------------------------------------------------------- #
        elif self.pick_and_twist_insert_fsm_state == PickAndPlaceState.PLACE_EXECUTION:
            desired_joint_ids = gripper_joint_ids
            num_gripper_joints = len(desired_joint_ids)
            desired_joint_position = torch.full(
                (num_gripper_joints,), 0.06, device=self.device
            )
            self.joint_position_command = desired_joint_position
            self.joint_command_ids = desired_joint_ids
            ...
            if self.pick_and_twist_insert_fsm_timer > TIME_CONSTANT_100:
                self.pick_and_twist_insert_fsm_timer = 0
                self.pick_and_twist_insert_fsm_state = PickAndPlaceState.PLACE_COMPLETE

        # -------------------------------------------------------------------------------------------------------------------------- #
        # [FSM Intermediate State] PLACE_COMPLETE ---------------------------------------------------------------------------------- #
        # -------------------------------------------------------------------------------------------------------------------------- #
        elif self.pick_and_twist_insert_fsm_state == PickAndPlaceState.PLACE_COMPLETE:
            desired_joint_position, desired_joint_ids = self.solve_inverse_kinematics( 
                arm_name=arm_name,
                target_ee_pos_b=target_place_ready_pos_b, 
                target_ee_quat_b=target_place_quat_b
            ...
            if (self.position_reached(ee_pos_w, target_place_ready_pos_w) and 
                ...
                self.pick_and_twist_insert_fsm_timer > TIME_CONSTANT_150):
                self.pick_and_twist_insert_fsm_timer = 0
                self.pick_and_twist_insert_fsm_state = PickAndPlaceState.FINALIZATION

        # -------------------------------------------------------------------------------------------------------------------------- #
        # [FSM End State] FINALIZATION --------------------------------------------------------------------------------------------- #
        # -------------------------------------------------------------------------------------------------------------------------- #
        elif self.pick_and_twist_insert_fsm_state == PickAndPlaceState.FINALIZATION:
            # [State Transition] FINALIZATION -> INITIALIZATION
            self.pick_and_twist_insert_fsm_timer += 1
            if self.pick_and_twist_insert_fsm_timer > TIME_CONSTANT_50:
                self.pick_and_twist_insert_fsm_timer = 0
                self.reset_pick_and_twist_insert()

    def reset_pick_and_place(self):
        self.pick_and_place_fsm_state = PickAndPlaceState.INITIALIZATION
        self.pick_and_place_fsm_timer = 0

    def reset_pick_and_twist_insert(self):
        self.pick_and_twist_insert_fsm_state = PickAndPlaceState.INITIALIZATION
        self.pick_and_twist_insert_fsm_timer = 0
